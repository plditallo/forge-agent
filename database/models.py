...
class AdminInvite(Base):
    """
    Single-use, time-boxed invite token for admin password disclosure.
    Generated by an already-authenticated admin from admin.html. Stored
    server-side (not in-process memory) so it survives Azure App Service
    restarts, idle-timeout recycles, and scaling events without losing
    outstanding invites.

    This does not create a new credential -- it gates revealing the one
    existing shared ADMIN_PASSWORD. A future version should move to
    per-admin credentials (see FORGE_FrontEnd_Debugging session notes,
    "real product" branch) so each admin is an independently revocable
    identity rather than one more holder of a single shared secret.
    """
    __tablename__ = "admin_invites"

    id                = Column(Integer, primary_key=True, index=True)
    token             = Column(String(64), unique=True, nullable=False, index=True)
    created_by_admin  = Column(String(255))
    created_at        = Column(DateTime, default=datetime.utcnow)
    expires_at        = Column(DateTime, nullable=False)
    used_at           = Column(DateTime, nullable=True)
    used_by_name      = Column(String(255), nullable=True)
